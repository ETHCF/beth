...
contract BETHReentryHarness is ERC20 {
    ...
    function setForwardTarget(address _target) external { forwardTarget = _target; }

    function deposit() external payable { _depositTo(msg.sender); }
    function depositTo(address recipient) external payable { _depositTo(recipient); }
    receive() external payable { _depositTo(msg.sender); }
